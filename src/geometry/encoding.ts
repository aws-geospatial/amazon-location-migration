...
export class MigrationEncoding {
   ...
   */
  static encodePath(path: google.maps.LatLng[] | google.maps.MVCArray<google.maps.LatLng>): string {
    ...
    const lngLatArray: [number, number][] = latLngs.map((latLng) => [latLng.lng(), latLng.lat()]);
